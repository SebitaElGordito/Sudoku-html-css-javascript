document.querySelector('#dark__mode-toggle').addEventListener('click', () => {
...
// screens
const start_screen = document.querySelector('#start__screen');
const game_screen = document.querySelector('#game__screen');
const pause_screen = document.querySelector('#pause__screen');
const result_screen = document.querySelector('#result__screen');
// ----------
const cells = document.querySelectorAll('.main__grid-cell');

const name_input = document.querySelector('#input__name');

const number_inputs = document.querySelectorAll('.number');

const player_name = document.querySelector('#player__name');
const game_level = document.querySelector('#game__level');
const game_time = document.querySelector('#game__time');

const result_time = document.querySelector('#result__time');
...
const checkErr = (value) => {
    const addErr = (cell) => {
        if (parseInt(cell.getAttribute('data-value')) === value) {
            ...
            cell.classList.add('cell__err');
            setTimeout(() => {
                cell.classList.remove('cell__err');
            }, 500);
        }
    }
    ...
    number_inputs.forEach((e, index) => {
        e.addEventListener('click', () => {
            if (!cells[selected_cell].classList.contains('filled')) {
                ...
                removeErr();
                checkErr(index + 1);
                cells[selected_cell].classList.add('zoom__in');
                setTimeout(() => {
                    cells[selected_cell].classList.remove('zoom__in');
                }, 500);
...
document.querySelector('#btn__play').addEventListener('click', () => {
    if (name_input.value.trim().length > 0) {
        ...
        name_input.classList.add('input__err');
        setTimeout(() => {
            name_input.classList.remove('input__err');
            name_input.focus();
        }, 500);
    }
});

document.querySelector('#btn__continue').addEventListener('click', () => {
    if (name_input.value.trim().length > 0) {
        loadSudoku();
        startGame();
    } else {
...
document.querySelector('#btn__new-game-2').addEventListener('click', () => {
    console.log('object')
    returnStartScreen();
});
